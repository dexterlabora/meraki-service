...
class merakiService {
   ...
   * @param {object=} proxy - the proxy used for the API call. See https://github.com/axios/axios#request-config for details on format. Uses default: {}
   * @returns {}
   */
  constructor(apiKey, baseUrl = 'https://api.meraki.com/api/v0', proxy = {}) {
    this._apiKey = process.env.API_KEY || apiKey;
    this._baseUrl = process.env.BASE_URL || baseUrl;
    this._proxy = proxy;
  ...
  initMeraki() {
    ...
    if (!_.isEmpty(this._proxy)) {
      opts.proxy = this._proxy
    }
    this.meraki = axios.create(opts);
